...
def get_dummyjson_products(limit=0):
    """
    Fetch products from DummyJSON API.
    `limit` controls how many products come back in one call
    (DummyJSON paginates; default API limit is 30).

    Design decision: default limit=0 tells DummyJSON to return its ENTIRE
    catalog (~194 products) instead of a partial page. We ran into a real
    bug during development where limit=100 silently excluded categories
    like "smartphones" that happened to sit outside the first 100
    products — a good example of why paginated APIs need care: a
    "successful" response can still be an incomplete one.
    """
